...
namespace Econolite.Ode.Messaging
{
    public class Source<TValue> : ISource<TValue>
    {
        ...
        private readonly SourceOptions<TValue> _options;
        private readonly IBuildMessagingConfig _buildMessagingConfig;
        private readonly IConsumeResultFactory<TValue> _consumeResultFactory;
        private readonly SemaphoreSlim _lock = new(1);

        public Source(IBuildMessagingConfig buildMessagingConfig, IConsumeResultFactory<TValue> consumeResultFactory, IOptions<SourceOptions<TValue>> options, ILoggerFactory loggerFactory)
        {
            _buildMessagingConfig = buildMessagingConfig;
            _consumeResultFactory = consumeResultFactory;
            _options = options?.Value ?? new SourceOptions<TValue>();
            _logger = loggerFactory.CreateLogger($"{GetType().Name}<{typeof(TValue).Name}>");
        }

        public Task ConsumeOnAsync(Func<Elements.ConsumeResult<Guid, TValue>, Task> consumeFunc, CancellationToken stoppingToken) => ConsumeOnAsync(_options.DefaultChannel, _options.DefaultTypeFilter, consumeFunc, stoppingToken);
        public Task ConsumeOnAsync(string channel, Func<Elements.ConsumeResult<Guid, TValue>, Task> consumeFunc, CancellationToken stoppingToken) => ConsumeOnAsync(channel, _options.DefaultTypeFilter, consumeFunc, stoppingToken);

        public async Task ConsumeOnAsync(string channel, Func<string, bool> typeFilter, Func<Elements.ConsumeResult<Guid, TValue>, Task> consumeFunc, CancellationToken stoppingToken)
        {
